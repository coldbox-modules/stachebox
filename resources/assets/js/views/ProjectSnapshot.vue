<template>
	<div class="border-gray-600 mb-3 project-snapshot">
		...
		<h3 class="text-gray-400 text-xl pb-2 mb-7 mt-7  border-gray-300 border-b">{{ $t( "Applications" ) }}:</h3>
		<template
		...
		<template v-if="project.beats.length">
			<h3 class="text-gray-400 text-xl font-medium pb-2 border-gray-300 border-b mt-7">{{ $t( "FileBeats" ) }}:</h3>
			<template v-for="(application, index) in project.beats">
				<h4 class="text-teal-400 text-md font-medium pb-2 border-gray-300 border-b">{{ application.toTitleCase() }}</h4>
				<beats-list
